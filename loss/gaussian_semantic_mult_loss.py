...
class GaussianSemanticMultLoss(BaseLoss):
    def __init__(self, 
                 weight=1.0,
                 num_classes=18,
    ...
    def gaussian_semantic_loss(self, gaussians, sampled_label, sampled_xyz, empty_label=17):
        """
        ...
        """
        total_loss = torch.tensor(0, device=gaussians[0].means.device)
        length = len(gaussians)
        ...
        for gaussian in gaussians:
            ...
            total_loss = total_loss + (semantic_loss + bce_loss) * self.weight
