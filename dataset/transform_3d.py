...
class DefaultFormatBundle(object):
    ...
    def __call__(self, results):
        ...
        if 'lidar_points' in results:
            lidar_points = results['lidar_points']
            if isinstance(lidar_points, np.ndarray):
                results['lidar_points'] = torch.from_numpy(lidar_points)
        
        # 处理位姿数据
        pose_keys = ['lidar_pose', 'ego_pose', 'lidar2img', 'ego2img']
...
class LoadLiDARFromFile(object):
    ...
    def __init__(self, pc_range=None, filter_points=True):
        self.pc_range = pc_range
    ...
    def __call__(self, results):
        ...
        if self.filter_points and self.pc_range is not None:
            # 过滤点云范围
        ...
        results['lidar_points'] = points.astype(np.float32)
        return results

    def __repr__(self):
        return self.__class__.__name__ + f'(pc_range={self.pc_range})'