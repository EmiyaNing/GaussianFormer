...
class GaussianHeadSemantic(BaseTaskHead):
    def __init__(
        self, 
        init_cfg=None,
        ...
        **kwargs,
        ...
        if use_localaggprob:
            ...
            from .localagg import local_aggregate
    ...
    def forward(
        ...
        **kwargs
        ...
        for idx in apply_loss_layers:
            ...
            one_hot_target = torch.zeros(
                *sampled_label.shape,
                self.num_classes, 
                dtype=torch.float32, 
                device=sampled_label.device)
            
            one_hot_target.scatter_(-1, sampled_label.unsqueeze(-1).long(), 1.0)

            gaussian_semantic_label = self.aggregator.inverse_render(
                sampled_xyz.clone().float(), 
                means,
                origi_opa.reshape(bs, g),
                one_hot_target,
        ...
        return {
            ...
            'gaussian_sem_label': gaussian_semantic_label.unsqueeze(0),
