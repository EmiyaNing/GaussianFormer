...
class DSVT(nn.Module):
    ...
    def __init__(self, 
        ...
        self.input_layer = DSVTInputLayer(model_cfg['input_layer'])
        block_name = model_cfg['block_name']
        set_info = model_cfg['set_info']
        d_model = model_cfg['d_model']
        nhead = model_cfg['nhead']
        dim_feedforward = model_cfg['dim_feedforward']
        dropout = model_cfg['dropout']
        activation = model_cfg['activation']
        self.reduction_type = model_cfg['reduction_type']
        # save GPU memory
        self.use_torch_ckpt = model_cfg['use_checkpoint']
        ...
        self.output_shape = model_cfg['output_shape']
        self.stage_num = stage_num
        self.set_info = set_info
        self.num_point_features = model_cfg['conv_out_channel']
...
class DSVTInputLayer(nn.Module):
    ...
    def __init__(self, model_cfg):
        super().__init__()
        self.model_cfg = model_cfg
        self.sparse_shape = model_cfg['sparse_shape']
        self.window_shape = model_cfg['window_shape']
        self.downsample_stride = model_cfg['downsample_stride'] 
        self.d_model = model_cfg['d_model']
        self.set_info = model_cfg['set_info']
        self.stage_num = len(self.d_model)
        self.hyprid_factor = model_cfg['hybrid_factor']

        self.window_shape = [[self.window_shape[s_id], [self.window_shape[s_id][coord_id] * self.hyprid_factor[coord_id] \
                                                for coord_id in range(3)]] for s_id in range(self.stage_num)]

        self.shift_list = model_cfg['shifts_list']
