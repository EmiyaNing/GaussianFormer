...
class GaussianLifterV2(BaseLifter):
    def __init__(
        ...
        **kwargs,
        ...
        image_anchor = torch.cat([scale, rots, opacity, semantic], dim=-1)
        ...
        if self.use_lidar_init:
            ...
            self.image_anchor = nn.Parameter(
            torch.tensor(image_anchor, dtype=torch.float32),
            requires_grad=anchor_grad,
            )
    ...
    def forward(self, metas, **kwargs):
        ...
        # 检查是否需要图像初始化填补
        need_image_fill = any(count < self.max_anchors for count in actual_anchor_counts)
        
        # 存储图像初始化的结果，用于损失计算
        image_based_result = None
        pixel_logits_list = []
        pixel_gt_list = []
        
        # 如果需要图像填补，准备图像初始化
        if need_image_fill:
            image_based_result = self._forward_image_based(metas, **kwargs)
            image_anchors = image_based_result['representation']
            image_features = image_based_result['rep_features']
            # 保存图像初始化的损失相关信息
            pixel_logits_list = [image_based_result['pixel_logits']] if image_based_result['pixel_logits'] is not None else []
            pixel_gt_list = [image_based_result['pixel_gt']] if image_based_result['pixel_gt'] is not None else []
        

        for b, (lidar_anchors, actual_count) in enumerate(zip(lidar_anchors_list, actual_anchor_counts)):
            # 如果实际锚点数小于最大锚点数，填充到最大数量
            # 如果实际锚点数小于最大锚点数，用图像初始化填补
            if actual_count < self.max_anchors:
                num_fill_needed = self.max_anchors - actual_count
            
                if need_image_fill and image_based_result is not None:
                    # 从图像初始化结果中选择填补的锚点，使用与原始图像初始化相同的逻辑
                    image_anchors_batch = image_anchors[b]
                    image_features_batch = image_features[b]
                    ...
                    print(f"Batch {b}: LiDAR锚点 {actual_count} + 图像锚点 {num_fill_needed} = 总计 {self.max_anchors}")
                else:
                    print("false")
        ...
        return {
            ...
            'pixel_logits': image_based_result['pixel_logits'],  # LiDAR初始化不需要深度估计
            'pixel_gt': image_based_result['pixel_gt'],      # LiDAR初始化不需要深度GT
        }
    ...
    def _forward_image_based(self, metas, **kwargs):
        ...
        anchor = torch.cat([
            xyz, torch.tile(self.image_anchor[None], (b, 1, 1))], dim=-1)
