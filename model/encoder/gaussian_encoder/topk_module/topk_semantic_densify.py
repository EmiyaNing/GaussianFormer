...
class SemanticDensify(BaseModule):
    '''
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super(SemanticDensify, self).__init__()
        ...
        self.feature_proj = nn.Sequential(
            nn.Linear(2 * feat_embed_dim, feat_embed_dim),
    ...
    def forward(
        ...
        for b in range(batch_size):
            ...
            # ball query to get the local relative points.
            _, idx, _, _  = frnn.frnn_grid_points(
                filter_means.unsqueeze(0), cur_means.unsqueeze(0), K = self.K, r=2, return_nn=True
            )
            # the gathered point feat with 1, 2560, 27, 128
            gathered_feats= frnn.frnn_gather(cur_feats.unsqueeze(0), idx)
            gathered_feats= gathered_feats.permute(0, 3, 1, 2)

            # maybe we could add the semantic gather in here

            # reback the local feats with shape 2560, 128
            local_feats = self.feature_agg(gathered_feats).permute(0, 2, 1, 3).squeeze(0).squeeze(-1)
            # cat the original feats with gathered feats
            filter_feats= torch.cat([local_feats, filter_feats], dim=-1)
            ...
            # refine module use the restrict xyz's moving 
            # use the unit is used for sigmoid process
